...
const hasVifChild = (childNodes) => childNodes.some((child) => {
  const props = getAttrs(child.rawAttrs)
  return props.hasOwnProperty('v-if');
});

const hasVelseChild = (childNodes) => childNodes.some((child) => {
  const props = getAttrs(child.rawAttrs)
...
async function parseJSXFile(argv) {
  ...
  if (matches && !!hasVIF) {
      ...
      const ret = str.replace(JSX_STRING, '<>' + root.outerHTML + '</>').replace('arrowfn', '=>').replace('&gt;', '>').replace('&lt;', '<');
