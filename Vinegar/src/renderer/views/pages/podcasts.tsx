...
export const Podcasts = () => {
  ...
  async function mounted() {
    const podcastShow = await app.mk.api.podcasts(`/v1/me/library/podcasts?include=episodes`);
  ...
  async function getEpisodes(podcast) {
    episodes = [];
    const eps = await app.mk.api.podcasts(`/v1/catalog/${app.mk.storefrontId}/podcasts/${podcast.id}?include=episodes`);
  ...
  async function getNextEpisodes(next, podcastId) {
    const podcastShow = await app.mk.api.podcasts(next);
